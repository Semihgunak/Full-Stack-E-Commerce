...
const CartTotals = () => {
  ...
  const user = localStorage.getItem("user")
    ...
    : null;
  const stripePuclicKey = import.meta.env.VITE_API_STRIPE_PUBLIC_KEY;
  ...
  const handlePayment = async () => {
    setLoading(true);
    if (!user) {
      return message.warning("You must log in to make a payment");
    }
    const body = {
      products: cartItems,
      user: user,
    ...
    try {
      const stripe = await loadStripe(stripePuclicKey);
      ...
      if (!res.ok) {
        return message.error("the payment transaction failed.");
      }
